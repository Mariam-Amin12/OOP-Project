...
#include"PickFigureColorAction.h"
...
PickFigureColorAction::PickFigureColorAction(ApplicationManager* pApp) :Action(pApp), LoadFile(pApp)
{
	CorrectCount = 0;
	InCorrectCount = 0;
...
void PickFigureColorAction::ReadActionParameters()
{
	pIn->GetPointClicked(p.x, p.y);
}


void PickFigureColorAction::Execute(bool x)
{
	pOut->ClearStatusBar();
	...
	NumberOfRandomFigures = pManager->NumOfRandomFigureColor(IndexOfRandomFigure, Type, FillColor); //Number of Random Chosen Figure in FigList
	...
	while (CorrectCount < NumberOfRandomFigures)
	{
		...
		if (Fig != NULL)
		{
			if (Type == Fig->GetType() && FillColor == Fig->GetFillingColor()) //Check if The Random Chosen Figure and the Selected Figure The same or not
				CorrectCount++;
			else
				InCorrectCount++;
